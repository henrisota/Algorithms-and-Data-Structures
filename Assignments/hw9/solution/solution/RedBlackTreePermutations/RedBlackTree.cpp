...
std::string RedBlackTree::inOrderTraversalRecursive(Node *x) {
    // Traverse the Red Black Tree recursively and read it inorder manner while
    // x is not a NIL element
    std::string result;
    if (x != this->NIL) {
        result += this->inOrderTraversalRecursive(x->left);
        result += std::to_string(x->data);
        if (x->color == RED)
            result += "R ";
        else result += "B ";
        result += this->inOrderTraversalRecursive(x->right);
    }
    return result;
}
...
void RedBlackTree::deleteFromTree(int key) {
...
std::string RedBlackTree::inOrder() {
    // Cal recursive helper function to read the under root node
    return this->inOrderTraversalRecursive(this->root);
}
